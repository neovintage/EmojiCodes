...
{
    NSMutableAttributedString *finalResult = [inString mutableCopy];
    
    NSRegularExpression *regex = [[NSRegularExpression alloc] initWithPattern:@"(:\\S+:)" options:NSRegularExpressionCaseInsensitive error:NULL];
    NSArray *results = [regex matchesInString:inString.string options:NSMatchingReportCompletion range:NSMakeRange(0, inString.string.length)];
    for (NSTextCheckingResult *result in [results reverseObjectEnumerator]) {
        NSString *code = [inString.string substringWithRange:result.range];
        NSString *unicode = self.emojiAliases[code];
        if (!unicode) continue;
        [finalResult replaceCharactersInRange:result.range withString:unicode];
	}
